...
built versions for `<PLUGIN_VERSION>` (e.g. `3.0.0-pr.17.1`) and `<MAGDA_VERSION>`
(e.g. `7.0.0-alpha.1`).
...
magda-auth-oidc:
  clientId: "magda-e2e-client"          # mock-oauth2-server accepts any client
...
magda:
  magda-core:
    gateway:
      authPlugins:
        - { key: oidc, baseUrl: http://magda-auth-oidc }
```

A fake client secret must exist: `kubectl create secret generic magda-auth-oidc-secret -n magda --from-literal=client-secret=e2e`.
...
```bash
kubectl get deploy magda-auth-oidc -n magda \
...
kubectl exec -n magda deploy/magda-auth-oidc -- node --input-type=module -e '
...
app.get("/w",(req,res)=>{ req.session.e2e="oidc-vf-"+Date.now(); res.end("ok"); });
...
```bash
IP=$(kubectl get pod -n magda -l service=magda-auth-oidc --field-selector=status.phase=Running -o jsonpath='{.items[0].status.podIP}')
...
```bash
kubectl port-forward -n magda svc/magda-auth-oidc 18200:80 &
...
curl -s -o /dev/null -D /tmp/authz.txt \
  --data-urlencode "username=magda-e2e-user" \
  --data-urlencode 'claims={"sub":"magda-e2e-user","email":"magda-e2e-user@example.com","name":"Magda E2E User"}' \
  "$AUTHZ"
...
# -> magda-e2e-user@example.com|Magda E2E User|oidc
```

## Result

Verified on minikube with Magda `7.0.0-alpha.1` and the plugin
(`3.0.0-pr.17.1`): under `verify-full` the plugin received the CA and its
...
`source=oidc` Magda user was created, and the authenticated session was
persisted.
...
kubectl delete secret pg-ca magda-auth-oidc-secret -n magda
